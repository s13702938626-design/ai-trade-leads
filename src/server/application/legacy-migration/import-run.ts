...
async function resolveAccount(item: LegacyMigrationItem, groups: Map<string, Account>, dependencies: Dependencies): Promise<{ account: Account; result: "created" | "linked" | "retained" }> {
  if (item.accountId) { const existing = await dependencies.accounts.getById(item.accountId, { includeDeleted: true }); if (!existing) throw new Error("Previously resolved account is missing."); return { account: existing, result: item.accountResult === "created" ? "created" : "retained" }; }
  ...
  try {
    ...
    for (const item of items) {
      ...
      try {
        ...
        await dependencies.migrations.markItemImported(resolvedItem.id, { accountId: resolved.account.id, evidenceId: linked.id, accountResult: resolved.result, evidenceResult: evidenceResult.status === "created" ? "created" : "duplicate" });
