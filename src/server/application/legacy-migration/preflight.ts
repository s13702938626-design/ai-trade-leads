...
function inspection(payload: unknown, index: number, seen: Set<string>, groups: Set<string>): LegacyLeadPreflight {
  const itemHash = sha256(JSON.stringify(payload));
  ...
  if (typeof payload.sourceUrl === "string" && payload.sourceUrl.trim()) { try { new URL(payload.sourceUrl); } catch { warnings.push("invalid_source_url"); } }
  ...
  if (typeof payload.fetchedAt === "string" && payload.fetchedAt.trim() && Number.isNaN(Date.parse(payload.fetchedAt))) warnings.push("invalid_fetched_at");
...
export async function preflightLegacyMigration(input: unknown, dependencies: { legacyMigrationRepository: any; accountRepository: any; clock: { now(): Date } }) {
  ...
  const items = await Promise.all(base.map(async (entry) => {
    ...
    const recommendedAction = candidates.some((candidate) => candidate.confidence === "high" && candidate.recommendation === "link_existing") ? "link_existing" : candidates.length ? "manual_review" : entry.recommendedAction;
