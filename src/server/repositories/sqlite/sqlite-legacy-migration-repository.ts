...
export class SqliteLegacyMigrationRepository {
  ...
  async markItemRolledBack(id) { this.c.db.update(legacyMigrationItems).set({ importStatus: "rolled_back", updatedAt: this.clock.now().getTime() }).where(eq(legacyMigrationItems.id, id)).run(); return (await this.getItemById(id))!; }
